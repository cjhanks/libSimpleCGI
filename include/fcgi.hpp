...
struct RawHeader {
    ...
    friend std::ostream& operator<<(std::ostream&, RawHeader&);
} __attribute__((packed));

using Header = RawHeader;

inline std::ostream& 
operator<<(std::ostream& strm, RawHeader& header)
{
    strm << std::endl
         << "Header:"
         << std::endl
...
inline Header
rawHeaderToNativeHeader(const RawHeader& header) {
    return (Header) { 
        .version       = header.version,
        .type          = header.type,
        .requestId     = __builtin_bswap16(header.requestId),
        .contentLength = __builtin_bswap16(header.contentLength),
...
inline bool
isManagementRecord(const Header& header)
{   
    return 0 == header.requestId; 
}
